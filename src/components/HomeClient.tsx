...
function HomeContent({ artworks }: { artworks: Artwork[] }) {
  ...
  const currentArtwork = artworks.find((a) => a.id === currentId);
  ...
  return (
    <>
      ...
      {/* Modale */}
      {currentId && (
        <ArtworkModal artwork={currentArtwork} onClose={closeArtwork} />
      )}
    </>
  );
}
